//
//  WalkingStepLengthChartCell.swift
//  MyHealthTrack
//
...
class WalkingStepLengthChartCell: UITableViewCell {
    ...
    let healthKitManager = DetailsWalkingStepLengthHealthKitManager()
    ...
    override func awakeFromNib() {
        ...
        lblTotal.text = "TOTAL"
        lblDay.text = "Today"
        let typesToRead: Set<HKSampleType> = [HKObjectType.quantityType(forIdentifier: .walkingStepLength)!]
            ...
            if success {
                self.healthKitManager.fetchDailyWalkingSpeed { (dailyData) in
                    ...
                    DispatchQueue.main.async {
                        self.updateBarChart()
                        self.lblData.text = "\(String(format: "%.2f", self.dailyTotalCount)) KM/H" //"\(MobilityHealthKitManager.todayWalkingStepLength!) KM/H"
                    }
                }
            } else {
                if let error = error {
                ...
                if success {
                    self.healthKitManager.fetchWeeklyWalkingSpeed{ (dailyData) in
                        ...
                        DispatchQueue.main.async {
                            ...
                            self.lblData.text = "\(String(format: "%.2f", self.weeklyTotalCount/7)) KM/H"
    ...
    func updateBarChart() {
        ...
        for (index, dataPoint) in dailyWalkingSpeedData.enumerated() {
            ...
            dailyTotalCount = dailyTotalCount + Double(Float(dataPoint.1))
