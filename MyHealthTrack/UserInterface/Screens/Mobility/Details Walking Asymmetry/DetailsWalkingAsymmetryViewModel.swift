//
//  DetailsWalkingAsymmetryViewModel.swift
//  MyHealthTrack
//
...
class DetailsWalkingAsymmetryHealthKitManager{
    ...
    func fetchDailyWalkingSpeed(completion: @escaping ([(Date, Double)]) -> Void) {
        ...
        let query = HKStatisticsCollectionQuery(quantityType: walkingAsymmetryType,
        ...
        query.initialResultsHandler = { query, result, error in
            ...
            var walkingAsymmetryData: [(Date, Double)] = []
            result.enumerateStatistics(from: startDate, to: now) { statistics, stop in
                if let averageSpeed = statistics.averageQuantity() {
                    let date = statistics.startDate
                    let speed = averageSpeed.doubleValue(for: HKUnit.percent())
                    walkingAsymmetryData.append((date, speed))
                }
            }

            completion(walkingAsymmetryData)
        }
