//
//  WalkingAsymmetryChartCell.swift
//  MyHealthTrack
//
...
class WalkingAsymmetryChartCell: UITableViewCell {
    ...
    let healthKitManager = DetailsWalkingAsymmetryHealthKitManager()
    ...
    override func awakeFromNib() {
        ...
        lblTotal.text = "TOTAL"
        lblDay.text = "Today"
        let typesToRead: Set<HKSampleType> = [HKObjectType.quantityType(forIdentifier: .walkingAsymmetryPercentage)!]
            ...
            if success {
                self.healthKitManager.fetchDailyWalkingSpeed { (dailyData) in
                    ...
                    DispatchQueue.main.async {
                        self.updateBarChart()
                        self.lblData.text = "\(String(format: "%.2f", self.dailyWalkingSpeedData)) %" //"\(MobilityHealthKitManager.todayWalkingAsymmetry!) %"
                    }
                }
            } else {
                if let error = error {
    ...
    @IBAction func segmentBar(_ sender: UISegmentedControl) {
                ...
                if success {
                    self.healthKitManager.fetchWeeklyWalkingSpeed{ (dailyData) in
                        ...
                        DispatchQueue.main.async {
                            ...
                            self.lblData.text = "\(String(format: "%.2f", self.weeklyTotalCount/7)) %"//"\(String(self.weeklyTotalCount/7)) %"
                ...
                if success {
                    self.healthKitManager.fetchMonthlyWalkingSpeed { (dailyData) in
                        ...
                        DispatchQueue.main.async {
                            ...
                            self.lblData.text = "\(String(format: "%.2f", self.monthlyTotalCount/30)) %"
        ...
        case 3:
            ...
            lblTotal.text = "Total" //"DAILY AVERAGE"
                ...
                if success {
                    self.healthKitManager.fetchHalfYearlyWalkingSpeed { (dailyData) in
                        ...
                        DispatchQueue.main.async {
                            ...
                            self.lblData.text = "\(String(format: "%.2f", self.halfYearlyTotalCount)) %"
                ...
                if success {
                    self.healthKitManager.fetchYearlyWalkingSpeed { (dailyData) in
                        ...
                        DispatchQueue.main.async {
                            ...
                            print(self.yearlyTotalCount)
                            self.lblData.text = "\(String(format: "%.2f", self.yearlyTotalCount)) %"
    ...
    func updateBarChart() {
        ...
        let chartDataSet = BarChartDataSet(entries: dataEntries, label: "Walking Asymmetry")
