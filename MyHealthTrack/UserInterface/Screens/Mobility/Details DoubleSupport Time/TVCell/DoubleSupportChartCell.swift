...
class DoubleSupportChartCell: UITableViewCell {
            ...
            if success {
                self.healthKitManager.fetchDailyStepCount { stepCountData in
                    ...
                    DispatchQueue.main.async {
                        self.lblData.text = "\(String(self.dailyTotalCount)) %" //"\(MobilityHealthKitManager.todayDoubleSupport!) Steps " //"\(HealthKitmanager.dailySteps!) Steps"
            ...
            healthKitManager.requestWeeklyAuthorization { success, _ in
                if success {
                    self.healthKitManager.fetchWeeklyStepCount { stepCountData in
                        ...
                        self.barChartView.xAxis.labelPosition = .bottom
                        DispatchQueue.main.async {
                            self.lblData.text = "\(String(self.weeklyTotalCount/7)) %"//"\(String(self.weeklyTotalCount/7)) Steps"
            ...
            healthKitManager.requestMonthlyAuthorization { success, _ in
                if success {
                    self.healthKitManager.fetchMonthlyStepCount { stepCountData in
                        ...
                        self.barChartView.xAxis.labelPosition = .bottom
                        DispatchQueue.main.async {
                            self.lblData.text = "\(String(self.monthlyTotalCount/30)) %"//"\(String(self.monthlyTotalCount/30)) Steps"
            ...
            healthKitManager.requestHalfYearlyAuthorization { success, _ in
                if success {
                    self.healthKitManager.fetchHalfYearlyStepCount { stepCountData in
                        ...
                        self.barChartView.xAxis.labelPosition = .bottom
                        DispatchQueue.main.async {
                            self.lblData.text = "\(String(self.halfYearlyTotalCount/180)) %"
            ...
            healthKitManager.requestYearlyAuthorization { success, _ in
                if success {
                    self.healthKitManager.fetchYearlyStepCount { stepCountData in
                        ...
                        self.barChartView.xAxis.labelPosition = .bottom
                        DispatchQueue.main.async {
                            self.lblData.text = "\(String(self.yearlyTotalCount/360)) %"
    ...
    func updateBarChart(with stepCountData: [(Date, Double)]) {
        ...
        let dataSet = BarChartDataSet(entries: entries, label: "Double Support")
        let data = BarChartData(dataSet: dataSet)
        dataSet.colors = [NSUIColor(cgColor: UIColor(red: 237.0/255.0, green: 127.0/255.0, blue: 51.0/255.0, alpha: 1.0).cgColor)]
