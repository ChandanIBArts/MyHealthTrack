//
//  DetailsWalkingSpeedViewModel.swift
//  MyHealthTrack
//
...
class DetailsWalkingSpeedHealthKitManager{
    ...
    func fetchDailyWalkingSpeed(completion: @escaping ([(Date, Double)]) -> Void) {
        ...
        let query = HKStatisticsCollectionQuery(quantityType: walkingSpeedType,
        ...
        query.initialResultsHandler = { query, result, error in
            ...
            var walkingSpeedData: [(Date, Double)] = []

            result.enumerateStatistics(from: startDate, to: now) { statistics, stop in
                if let averageSpeed = statistics.averageQuantity() {
                    let date = statistics.startDate
                    let speed = averageSpeed.doubleValue(for: HKUnit.meter().unitDivided(by: .hour()))
                    walkingSpeedData.append((date, speed))
                }
            }

            completion(walkingSpeedData)
        }
