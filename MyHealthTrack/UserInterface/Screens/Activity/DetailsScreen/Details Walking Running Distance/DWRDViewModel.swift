...
class DWRDHealthKitManager {
    ...
    func fetchDailyStepCount(completion: @escaping ([(Date, Double)]) -> Void) {
        guard let distanceCountType = HKQuantityType.quantityType(forIdentifier: .distanceWalkingRunning) else {
            completion([])
            return
        }
        ...
        let query = HKStatisticsCollectionQuery(quantityType: distanceCountType,
        ...
        query.initialResultsHandler = { query, result, error in
            ...
            var distanceCountData: [(Date, Double)] = []

            result.enumerateStatistics(from: startDate, to: endDate) { statistics, stop in
                if let sum = statistics.sumQuantity() {
                    let date = statistics.startDate
                    let unit = sum.doubleValue(for: HKUnit.meterUnit(with: .kilo))
                    distanceCountData.append((date, unit))
                }
            }

            completion(distanceCountData)
        }
