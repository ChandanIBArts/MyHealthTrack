...
class HeightChartCell: UITableViewCell {
    ...
    let healthKitManager = DetailsHeightKitManager()
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        setUpUi()
        let typesToRead: Set<HKSampleType> = [HKObjectType.quantityType(forIdentifier: .height)!]
    ...
    func updateBarChart() {
        ...
        for (index, dataPoint) in dailyWalkingSpeedData.enumerated() {
            ...
            dailyData = dailyData + Int(dataPoint.1 * 100)
            count = index + index
            dataEntries.append(entry)
        }
        
        if count == 0 {
            self.dataLbl.text = "\(String(self.dailyData)) Centimeter"
            dailyData = 0
        } else {
            self.dataLbl.text = "\(String(self.dailyData / count)) Centimeter"
            dailyData = 0
        }
    ...
    func updateBarChart1() {
        ...
        for (index, dataPoint) in dailyWalkingSpeedData.enumerated() {
            let entry = BarChartDataEntry(x: Double(index), y: dataPoint.1)
            weeklyData = weeklyData + Int(dataPoint.1 * 100)
            count = index + index
            dataEntries.append(entry)
        }
        if count == 0 {
            self.dataLbl.text = "\(String(self.weeklyData)) Centimeter"
            weeklyData = 0
        } else {
            self.dataLbl.text = "\(String(self.weeklyData/count)) Centimeter"
            weeklyData = 0
        }
        let chartDataSet = LineChartDataSet(entries: dataEntries, label: "Height")
        let chartData = LineChartData(dataSet: chartDataSet)
        chartDataSet.colors = [UIColor.purple]
        chartDataSet.circleColors = [UIColor.purple]
        lineChartView.data = chartData
    ...
    func updateBarChart2() {
        ...
        for (index, dataPoint) in dailyWalkingSpeedData.enumerated() {
            let entry = BarChartDataEntry(x: Double(index), y: dataPoint.1)
            monthlyData = monthlyData + Int(dataPoint.1 * 100)
            count = index + index
            dataEntries.append(entry)
        }
        if count == 0 {
            self.dataLbl.text = "\(String(self.monthlyData)) Centimeter"
            monthlyData = 0
        } else {
            self.dataLbl.text = "\(String(self.monthlyData/count)) Centimeter"
            monthlyData = 0
        }
        let chartDataSet = LineChartDataSet(entries: dataEntries, label: "Height")
        let chartData = LineChartData(dataSet: chartDataSet)
    ...
    func updateBarChart3() {
        ...
        for (index, dataPoint) in dailyWalkingSpeedData.enumerated() {
            let entry = BarChartDataEntry(x: Double(index), y: dataPoint.1)
            halfYearlyData = halfYearlyData + Int(dataPoint.1 * 100)
            count = index + index
            dataEntries.append(entry)
        }
        if count == 0 {
            self.dataLbl.text = "\(String(self.halfYearlyData)) Centimeter"
            halfYearlyData = 0
        } else {
            self.dataLbl.text = "\(String(self.halfYearlyData/count)) Centimeter"
            halfYearlyData = 0
        }
        let chartDataSet = LineChartDataSet(entries: dataEntries, label: "Height")
        let chartData = LineChartData(dataSet: chartDataSet)
    ...
    func updateBarChart4() {
        ...
        for (index, dataPoint) in dailyWalkingSpeedData.enumerated() {
            let entry = BarChartDataEntry(x: Double(index), y: dataPoint.1)
            yearlyData = yearlyData + Int(dataPoint.1 * 100)
            count = index + index
            dataEntries.append(entry)
        }
        if count == 0 {
            self.dataLbl.text = "\(String(self.yearlyData)) Centimeter"
            yearlyData = 0
        } else {
            self.dataLbl.text = "\(String(self.yearlyData/count)) Centimeter"
            yearlyData = 0
        }
        let chartDataSet = LineChartDataSet(entries: dataEntries, label: "Height")
        let chartData = LineChartData(dataSet: chartDataSet)
